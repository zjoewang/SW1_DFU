...
/** MAX17055 class.
 *  Used for controlling a MAX17055 fuel gauge connected via I2C.
 *
 * Example:
 * @code
 * #include "mbed.h"
 * #include "MAX17055.h"
 *
 * MAX17055 gauge(p28, p27);
 *
 * int main()
 * {
 *     //Try to open the MAX17055
 *     if (gauge.open()) {
...
{
public:
    /** The default compensation value for the MAX17055
     */
    static const char RCOMP0;

    /** Represents the different alert flags for the MAX17055
     */
    enum AlertFlags {
    ...
    /** Create a MAX17055 object connected to the specified I2C pins
     *
     * @param sda The I2C data pin.
     * @param scl The I2C clock pin.
     * @param hz The I2C bus frequency (defaults to 400kHz).
     */
    MAX17055(PinName sda, PinName scl, int hz = 400000);

    /** Probe for the MAX17055 and indicate if it's present on the bus
     *
     * @returns
    ...
    /** Command the MAX17055 to perform a power-on reset
     */
    void reset();

    /** Command the MAX17055 to perform a QuickStart
     */
    void quickStart();

    /** Determine whether sleep mode is enabled on the MAX17055
     *
     * @returns
    ...
    /** Enable or disable sleep mode on the MAX17055
     *
     * @param enabled Whether or not sleep mode is enabled.
     */
    void sleepEnabled(bool enabled);

    /** Determine whether or not the MAX17055 is hibernating
     *
     * @returns
     *   'true' if hibernating,
    ...
    /** Enter or exit sleep mode on the MAX17055 (sleep mode must be enabled first)
     *
     * @param sleep Whether or not to sleep.
     */
    void sleep(bool sleep);

    /** Determine whether or not the SOC 1% change alert is enabled on the MAX17055
     *
     * @returns
     *   'true' if enabled,
    ...
    /** Enable or disable the SOC 1% change alert on the MAX17055
     *
     * @param enabled Whether or the SOC 1% change alert is enabled.
     */
    void socChangeAlertEnabled(bool enabled);

    /** Determine whether or not the MAX17055 is asserting the ALRT pin
     *
     * @returns
     *   'true' if alerting,
    ...
    /** Enable or disable the reset voltage comparator on the MAX17055
     *
     * @param enabled Whether or not the reset voltage comparator is enabled.
     */
    void comparatorEnabled(bool enabled);

    /** Get the factory programmed 8-bit ID of the MAX17055
     *
     * @returns The 8-bit ID.
     */
    char id();

    /** Determine whether or not the voltage reset alert is enabled on the MAX17055
     *
     * @returns
     *   'true' if enabled,
     *   'false' if not enabled.
